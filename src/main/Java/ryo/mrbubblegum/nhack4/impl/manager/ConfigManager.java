...
public class ConfigManager
    ...
    public String config = "nhack4/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                Loader.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Loader.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("nhack4").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("nhack4/" + name + "/")) ? "nhack4/" + name + "/" : "nhack4/config/";
    ...
    public void saveConfig(String name) {
        ...
        this.config = "nhack4/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        Loader.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("nhack4/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("nhack4", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Loader.moduleManager.modules);
        this.features.add(Loader.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Loader.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Loader.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
