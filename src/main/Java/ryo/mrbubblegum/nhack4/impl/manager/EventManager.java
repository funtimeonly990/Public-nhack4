...
import ryo.mrbubblegum.nhack4.impl.util.GLUProjection;
...
import ryo.mrbubblegum.nhack4.world.events.*;
...
public class EventManager
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!EventManager.fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(EventManager.mc.player)) {
            Loader.potionManager.update();
            Loader.totemPopManager.onUpdate();
            Loader.inventoryManager.update();
            Loader.holeManager.update();
            Loader.safetyManager.onUpdate();
            Loader.moduleManager.onUpdate();
            Loader.timerManager.update();
            if (this.timer.passedMs(Managers.getInstance().moduleListUpdates.getValue().intValue())) {
                Loader.moduleManager.sortModules(true);
                Loader.moduleManager.alphabeticallySortModules();
                this.timer.reset();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        Loader.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        Loader.moduleManager.onLogout();
        Loader.totemPopManager.onLogout();
        Loader.potionManager.onLogout();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            Loader.baritoneManager.onUpdateWalkingPlayer();
            Loader.speedManager.updateValues();
            Loader.rotationManager.updateRotations();
            Loader.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            Loader.rotationManager.restoreRotations();
            Loader.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        Loader.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(EventManager.mc.world) instanceof EntityPlayer) {
                ...
                Loader.totemPopManager.onTotemPop(player);
                Loader.potionManager.onTotemPop(player);
            }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        EventManager.mc.profiler.startSection("nhack4");
        ...
        Loader.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            Loader.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            Loader.moduleManager.onRender2D(render2DEvent);
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    Loader.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
