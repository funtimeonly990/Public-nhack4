...
import ryo.mrbubblegum.nhack4.injections.mixins.accessors.IEntityLivingBase;
import ryo.mrbubblegum.nhack4.lite.client.Managers;
import ryo.mrbubblegum.nhack4.lite.player.Blink;
...
class EntityUtil implements Util {
    ...
    public static List<EntityPlayer> getNearbyPlayers(double d) {
        ...
        List<EntityPlayer> list = mc.world.playerEntities.stream().filter(entityPlayer -> mc.player != entityPlayer).filter(entityPlayer -> (double) mc.player.getDistance(entityPlayer) <= d).filter(entityPlayer -> !(EntityUtil.getHealth(entityPlayer) < 0.0f)).collect(Collectors.toList());
        list.removeIf(entityPlayer -> Loader.friendManager.isFriend(entityPlayer.getName()));
        return list;
    }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(mc.player) || (entity instanceof EntityPlayer && Loader.friendManager.isFriend(entity.getName())) || mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Loader.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : mc.world.playerEntities) {
            ...
            output.put(healthSB + " " + (Loader.friendManager.isFriend(player) ? "\u00A7b" : "\u00A7r") + player.getName() + " " + distanceSB + " " + "\u00A7f" + Loader.totemPopManager.getTotemPopString(player) + Loader.potionManager.getTextRadarPotion(player), (int) mc.player.getDistance(player));
