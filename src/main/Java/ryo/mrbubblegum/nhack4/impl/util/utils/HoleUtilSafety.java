...
public class HoleUtilSafety
    ...
    public static boolean isObbyHole(BlockPos blockPos) {
        ...
        for (BlockPos blockPos2 : holeOffsets) {
            Block block = mc.world.getBlockState(blockPos.add(blockPos2)).getBlock();
            if (!HoleUtilSafety.isSafeBlock(blockPos.add(blockPos2))) {
                bl = false;
                continue;
            }
        ...
        if (mc.world.getBlockState(blockPos.add(0, 2, 0)).getBlock() != Blocks.AIR || mc.world.getBlockState(blockPos.add(0, 1, 0)).getBlock() != Blocks.AIR) {
            bl = false;
        }
        if (n < 1) {
            bl = false;
    ...
    public static boolean isHole(BlockPos blockPos) {
        ...
        int n = 0;
        for (BlockPos blockPos2 : holeOffsets) {
            if (mc.world.getBlockState(blockPos.add(blockPos2)).getMaterial().isReplaceable()) continue;
            ++n;
        }
        if (n == 5) {
            bl = true;
    ...
    public static List<Hole> getHoles(double d, BlockPos blockPos, boolean bl) {
        ...
        for (BlockPos blockPos2 : list) {
            Hole hole;
            if (mc.world.getBlockState(blockPos2).getBlock() != Blocks.AIR) continue;
            ...
            if (!bl || (hole = HoleUtilSafety.isDoubleHole(blockPos2)) == null || mc.world.getBlockState(hole.pos1.add(0, 1, 0)).getBlock() != Blocks.AIR && mc.world.getBlockState(hole.pos2.add(0, 1, 0)).getBlock() != Blocks.AIR)
                continue;
            arrayList.add(hole);
        }
        return arrayList;
        ...
        for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
            ...
            if (HoleUtilSafety.isAir(blockPos2)) {
                ...
                for (EnumFacing enumFacing2 : EnumFacing.HORIZONTALS) {
                    ...
                    IBlockState iBlockState = mc.world.getBlockState(blockPos2.offset(enumFacing2));
        ...
        for (BlockPos blockPos2 : hashSet) {
            if (HoleUtilSafety.isAir(blockPos2) && HoleUtilSafety.isAir(blockPos2.up()) && !HoleUtilSafety.isAir(blockPos2.down())) {
                ...
                for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
                    ...
                    if (hashSet.contains(blockPos3)) continue;
                    IBlockState iBlockState = mc.world.getBlockState(blockPos3);
                    if (!Arrays.stream(NO_BLAST).noneMatch(block -> block == iBlockState.getBlock())) continue;
                    return false;
                }
                continue;
    ...
    static boolean isSafeBlock(BlockPos blockPos) {
        return mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN || mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || mc.world.getBlockState(blockPos).getBlock() == Blocks.ENDER_CHEST;
    }
    ...
    public static boolean checkOffset(BlockPos blockPos, int n, int n2) {
        return mc.world.getBlockState(blockPos).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(n, 0, n2)).getBlock() == Blocks.AIR && HoleUtilSafety.isSafeBlock(blockPos.add(0, -1, 0)) && HoleUtilSafety.isSafeBlock(blockPos.add(n, -1, n2)) && HoleUtilSafety.isSafeBlock(blockPos.add(n * 2, 0, n2 * 2)) && HoleUtilSafety.isSafeBlock(blockPos.add(-n, 0, -n2)) && HoleUtilSafety.isSafeBlock(blockPos.add(n2, 0, n)) && HoleUtilSafety.isSafeBlock(blockPos.add(-n2, 0, -n)) && HoleUtilSafety.isSafeBlock(blockPos.add(n, 0, n2).add(n2, 0, n)) && HoleUtilSafety.isSafeBlock(blockPos.add(n, 0, n2).add(-n2, 0, -n));
    }
    ...
    public static boolean[] is1x1(BlockPos blockPos, boolean[] arrbl) {
        for (EnumFacing enumFacing : EnumFacing.values()) {
            ...
            if (enumFacing == EnumFacing.UP || (iBlockState = mc.world.getBlockState(blockPos2 = blockPos.offset(enumFacing))).getBlock() == Blocks.BEDROCK)
                continue;
