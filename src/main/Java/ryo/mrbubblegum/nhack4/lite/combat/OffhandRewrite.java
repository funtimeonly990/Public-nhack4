...
public class OffhandRewrite
    ...
    public Setting<Mode2> offhandmode = this.register(new Setting<Object>("Offhand", Mode2.TOTEMS, v -> this.pageSetting.getValue() == page.MAIN));
    ...
    public Setting<Bind> SwitchBind = this.register(new Setting<Object>("SwitchKey", new Bind(-1), v -> this.switchmode.getValue() && this.pageSetting.getValue() == page.MAIN));
    ...
    public Setting<Integer> falldistance = this.register(new Setting<Object>("FallDistance", Integer.valueOf(100), Integer.valueOf(1), Integer.valueOf(100), v -> this.fallcheck.getValue() && this.pageSetting.getValue() == page.MISC));
    ...
    public Setting<Integer> pingvalue = this.register(new Setting<Object>("Ping Value", Integer.valueOf(200), Integer.valueOf(0), Integer.valueOf(1000), v -> this.antiPing.getValue() && this.pageSetting.getValue() == page.MISC));
    ...
    public void onUpdateWalkingPlayer(ProcessRightClickBlockEvent event) {
        if (event.hand == EnumHand.MAIN_HAND && event.stack.getItem() == Items.END_CRYSTAL && OffhandRewrite.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && OffhandRewrite.mc.objectMouseOver != null && event.pos == OffhandRewrite.mc.objectMouseOver.getBlockPos()) {
            ...
            OffhandRewrite.mc.playerController.processRightClick(OffhandRewrite.mc.player, OffhandRewrite.mc.world, EnumHand.OFF_HAND);
        }
    }
        ...
        if (Keyboard.getEventKeyState() && this.switchmode.getValue().booleanValue() && this.SwitchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchval < this.maxSwitch.getValue()) {
                ...
                if (this.debug.getValue().booleanValue()) {
                    ...
                    Notifications.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(textComponentString, this.s.length() * 10);
                }
                this.doSwitch();
            }
            if (this.debug.getValue().booleanValue()) {
        ...
        if (!Feature.fullNullCheck() && OffhandRewrite.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && OffhandRewrite.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && OffhandRewrite.mc.gameSettings.keyBindUseItem.isKeyDown()) {
            if (event.getPacket() instanceof CPacketPlayerTryUseItemOnBlock) {
                ...
                if (packet2.getHand() == EnumHand.MAIN_HAND) {
                    if (this.timer.passedMs(50L)) {
                        OffhandRewrite.mc.player.setActiveHand(EnumHand.OFF_HAND);
                        OffhandRewrite.mc.player.connection.sendPacket(new CPacketPlayerTryUseItem(EnumHand.OFF_HAND));
                    }
                    event.setCanceled(true);
                }
    ...
    public void doSwitch() {
        ...
        } else if (this.rightGap.getValue().booleanValue() && OffhandRewrite.mc.player.getHealth() > this.switchHp.getValue().floatValue() && OffhandRewrite.mc.gameSettings.keyBindUseItem.isKeyDown() && OffhandRewrite.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword && !(OffhandRewrite.mc.currentScreen instanceof GuiContainer) && !(OffhandRewrite.mc.currentScreen instanceof GuiChat) && !(OffhandRewrite.mc.currentScreen instanceof LiteGui)) {
            this.currentMode = Mode2.GAPPLES;
        } else if (this.currentMode != Mode2.CRYSTALS && this.offhandmode.getValue() == Mode2.CRYSTALS && (EntityUtil.isSafe(OffhandRewrite.mc.player) && EntityUtil.getHealth(OffhandRewrite.mc.player, true) > this.holeHP.getValue().floatValue() || EntityUtil.getHealth(OffhandRewrite.mc.player, true) > this.switchHp.getValue().floatValue())) {
            this.currentMode = Mode2.CRYSTALS;
        } else if (this.currentMode != Mode2.GAPPLES && this.offhandmode.getValue() == Mode2.GAPPLES && (EntityUtil.isSafe(OffhandRewrite.mc.player) && EntityUtil.getHealth(OffhandRewrite.mc.player, true) > this.holeHP.getValue().floatValue() || EntityUtil.getHealth(OffhandRewrite.mc.player, true) > this.switchHp.getValue().floatValue())) {
            this.currentMode = Mode2.GAPPLES;
        }
        ...
        if (this.currentMode == Mode2.CRYSTALS && (!EntityUtil.isSafe(OffhandRewrite.mc.player) && EntityUtil.getHealth(OffhandRewrite.mc.player, true) <= this.switchHp.getValue().floatValue() || EntityUtil.getHealth(OffhandRewrite.mc.player, true) <= this.holeHP.getValue().floatValue())) {
        ...
        if (this.switchedForHealthReason && (EntityUtil.isSafe(OffhandRewrite.mc.player) && EntityUtil.getHealth(OffhandRewrite.mc.player, true) > this.holeHP.getValue().floatValue() || EntityUtil.getHealth(OffhandRewrite.mc.player, true) > this.switchHp.getValue().floatValue())) {
        ...
        if (this.antiPing.getValue().booleanValue() && (PingBypass.getInstance().isConnected() ? PingBypass.getInstance().getServerPing() : (long) Loader.serverManager.getPing()) > (long) this.pingvalue.getValue().intValue()) {
            this.setMode(Mode2.TOTEMS);
        }
        if (this.lagSwitch.getValue().booleanValue() && Loader.serverManager.isServerNotResponding()) {
            this.setMode(Mode2.TOTEMS);
        }
    ...
    public boolean calcCrystal() {
        for (Entity t : OffhandRewrite.mc.world.loadedEntityList) {
            if (!(t instanceof EntityEnderCrystal) || !(OffhandRewrite.mc.player.getDistanceSq(t.getPosition()) <= 36.0) || !(DamageUtil.calculateDamage(t, OffhandRewrite.mc.player) >= OffhandRewrite.mc.player.getHealth()))
                continue;
            return true;
        }
        return false;
