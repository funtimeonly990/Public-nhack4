...
import ryo.mrbubblegum.nhack4.injections.mixins.accessors.IContainer;
import ryo.mrbubblegum.nhack4.injections.mixins.accessors.ISPacketSetSlot;
...
public class Offhand
    ...
    public Setting<Boolean> onlyWTotem = this.register(new Setting<Object>("OnlyWTotem", Boolean.valueOf(true), v -> this.autoGapple.getValue() && this.type.getValue() == Type.OLD));
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && !Loader.friendManager.isFriend(e) && Offhand.mc.player.getDistance(e) <= this.cTargetDistance.getValue().floatValue());
    }
