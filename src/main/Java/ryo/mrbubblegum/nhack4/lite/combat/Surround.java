...
public class Surround
    ...
    private final ryo.mrbubblegum.nhack4.impl.util.Timer timer = new ryo.mrbubblegum.nhack4.impl.util.Timer();
    private final ryo.mrbubblegum.nhack4.impl.util.Timer retryTimer = new Timer();
    ...
    public Surround() {
        super("Surround", "obby required", Module.Category.COMBAT, true, false, false);
    }
    ...
    public void onEnable() {
        ...
        switch (this.centerType.getValue()) {
            case Instant: {
                Loader.positionManager.setPositionPacket((double) this.startPos.getX() + 0.5, this.startPos.getY(), (double) this.startPos.getZ() + 0.5, true, true, true);
            }
            case Motion: {
                Loader.movementManager.setMotion(((double) this.startPos.getX() + 0.5 - Surround.mc.player.posX) / 2.0, Surround.mc.player.motionY, ((double) this.startPos.getZ() + 0.5 - Surround.mc.player.posZ) / 2.0);
            }
        }
        this.retries.clear();
    ...
    private void doFeetPlace() {
        ...
        if (!EntityUtils.isSafe(Surround.mc.player, 0, true)) {
            this.isSafe = 0;
            this.placeBlocks(Surround.mc.player.getPositionVector(), EntityUtils.getUnsafeBlockArray(Surround.mc.player, 0, true), true, false, false);
        } else if (!EntityUtils.isSafe(Surround.mc.player, -1, false)) {
            this.isSafe = 1;
            this.placeBlocks(Surround.mc.player.getPositionVector(), EntityUtils.getUnsafeBlockArray(Surround.mc.player, -1, false), false, false, true);
        } else {
            this.isSafe = 3;
    ...
    private Vec3d areClose(Vec3d[] vec3ds) {
        ...
        for (Vec3d vec3d : vec3ds) {
            for (Vec3d pos : EntityUtils.getUnsafeBlockArray(Surround.mc.player, 0, true)) {
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, false)) {
                case 1: {
                    ...
                    if (Loader.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= 1) continue block5;
