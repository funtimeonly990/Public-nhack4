...
public class Killaura
    ...
    public Setting<Float> raytrace = this.register(new Setting<Object>("Raytrace", Float.valueOf(6.0f), Float.valueOf(0.1f), Float.valueOf(7.0f), v -> !this.teleport.getValue(), "Wall Range."));
    ...
    public Killaura() {
        super("Aura", "killaura", Module.Category.COMBAT, true, false, false);
    }
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue().booleanValue()) {
            if (this.stay.getValue().booleanValue() && target != null) {
                Loader.rotationManager.lookAtEntity(target);
            }
            this.doKillaura();
        }
    }
    ...
    private void doKillaura() {
        ...
        int wait = this.oneDotEight.getValue().booleanValue() ? 0 : (wait = (int) ((float) DamageUtil.getCooldownByWeapon(Killaura.mc.player) * (this.tps.getValue() ? Loader.serverManager.getTpsFactor() : 1.0f)));
        ...
        if (this.teleport.getValue().booleanValue()) {
            Loader.positionManager.setPositionPacket(Killaura.target.posX, EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.posY : Killaura.target.posY + (double) target.getEyeHeight(), Killaura.target.posZ, true, true, !this.lagBack.getValue());
        }
    ...
    private Entity getTarget() {
        Entity target = null;
        double distance = this.teleport.getValue() ? (double) this.teleportRange.getValue().floatValue() : (double) this.range.getValue().floatValue();
        double maxHealth = 36.0;
        for (Entity entity : Killaura.mc.world.loadedEntityList) {
            if (!(this.players.getValue() && entity instanceof EntityPlayer || this.animals.getValue() && EntityUtil.isPassive(entity) || this.mobs.getValue() && EntityUtil.isMobAggressive(entity) || this.vehicles.getValue() && EntityUtil.isVehicle(entity)) && (!this.projectiles.getValue().booleanValue() || !EntityUtil.isProjectile(entity)) || entity instanceof EntityLivingBase && EntityUtil.isntValid(entity, distance) || !this.teleport.getValue().booleanValue() && !Killaura.mc.player.canEntityBeSeen(entity) && !EntityUtil.canEntityFeetBeSeen(entity) && Killaura.mc.player.getDistanceSq(entity) > MathUtil.square(this.raytrace.getValue().floatValue()))
                continue;
            if (target == null) {
                target = entity;
