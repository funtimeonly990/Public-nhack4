...
public class HoleFiller
    ...
    public HoleFiller() {
        super("HoleFill", "fill holes using obby", Module.Category.COMBAT, true, false, true);
        this.setInstance();
    }
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            Loader.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            Loader.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = Loader.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Loader.holeManager.getMidSafety());
            }
        }
        object = Loader.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Loader.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Loader.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }
