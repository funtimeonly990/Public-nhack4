...
public class HUD
    ...
    private static final ResourceLocation codHitmarker = new ResourceLocation("nhack4sounds", "cod_hitmarker");
    private static final ResourceLocation csgoHitmarker = new ResourceLocation("nhack4sounds", "csgo_hitmarker");
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "NHACK4", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public Setting<Integer> hudRed = this.register(new Setting<Object>("Red", Integer.valueOf(135), Integer.valueOf(0), Integer.valueOf(255), v -> !this.rainbow.getValue()));
    public Setting<Integer> hudGreen = this.register(new Setting<Object>("Green", Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(255), v -> !this.rainbow.getValue()));
    public Setting<Integer> hudBlue = this.register(new Setting<Object>("Blue", Integer.valueOf(255), Integer.valueOf(0), Integer.valueOf(255), v -> !this.rainbow.getValue()));
    ...
    public void onUpdate() {
        for (final Module module : Loader.moduleManager.sortedModules) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        if (Objects.requireNonNull(this.watermark.getValue()) == WaterMark.NHACK4) {
            this.renderer.drawString("NHACK4 " + (this.modeVer.getValue() ? Loader.MODVER : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
        }
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Loader.moduleManager.alphabeticallySortedModules.size() : Loader.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Loader.moduleManager.alphabeticallySortedModules.get(k) : Loader.moduleManager.sortedModules.get(k);
                    ...
                    final Color moduleColor = Loader.moduleManager.moduleColorMap.get(module);
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Loader.moduleManager.alphabeticallySortedModules.size() : Loader.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Loader.moduleManager.alphabeticallySortedModules.get(Loader.moduleManager.alphabeticallySortedModules.size() - 1 - k) : Loader.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + ChatFormatting.GRAY + ((module.getDisplayInfo() != null) ? (" [" + ChatFormatting.WHITE + module.getDisplayInfo() + ChatFormatting.GRAY + "]") : "");
                    final Color moduleColor = Loader.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + ChatFormatting.WHITE + Loader.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : Loader.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? Loader.potionManager.getPotionString(effect) : Loader.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + ChatFormatting.WHITE + Loader.speedManager.getSpeedKpH() + " km/h";
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + ChatFormatting.WHITE + Loader.serverManager.getTPS();
            ...
            final String text = grayString + "Ping " + ChatFormatting.WHITE + (PingBypass.getInstance().isConnected() ? PingBypass.getInstance().getServerPing() : Loader.serverManager.getPing()) + (this.MS.getValue() ? "ms" : "");
            ...
            final String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            final String text = grayString + "Ping " + ChatFormatting.WHITE + Loader.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        final String text4 = (this.direction.getValue() ? (Loader.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to NHACK4 " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        ...
        if (Loader.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? ChatFormatting.GRAY : ChatFormatting.RED) + "Server not responding: " + MathUtil.round(Loader.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";
